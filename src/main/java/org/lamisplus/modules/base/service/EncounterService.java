...
import org.lamisplus.modules.base.service.providers.EncounterProcessor;
        ...
        */
        ...
        if(encounterDTO.getData().size() >0){
            encounterDTO.getData().forEach(formDataList -> {
                ...
                Encounter encounter1 = this.encounterRepository.getOne(savedEncounter.getId());
                formData.setEncounter(encounter1);
                formData.setData(formDataList);
                this.formDataRepository.save(formData);
            });
        }

        applicationContext.getBeansOfType(EncounterProcessor.class)
                .forEach((n, t)-> {
                    try{
                        t.process(savedEncounter);
